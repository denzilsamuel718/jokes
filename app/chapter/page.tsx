...
const thalaivar={image:"/friends/thalaivar/card.jpeg",memoryImage:"/friends/thalaivar/memory.jpeg",chapterImage:"/friends/thalaivar/chapter.jpeg",cornerImage:"/friends/thalaivar/corner.jpeg"};
const fallback:Story={group:"JOKES",friends:[{id:1,name:"Thalaivar",nickname:"The spark",note:"Add a short line about this friend.",memory:"Write a favorite memory, funny moment or inside joke here.",color:colors[0],specialImage:thalaivar.chapterImage,...thalaivar},...["The calm","The storyteller","The compass","The wildcard"].map((nickname,i)=>({id:i+2,name:`Friend ${i+2}`,nickname,note:"Add a short line about this friend.",memory:"Write a favorite memory, funny moment or inside joke here.",color:colors[i+1]}))]};
export default function Chapter(){
 ...
 useEffect(()=>{const id=Number(new URLSearchParams(location.search).get("id"));let story=fallback;try{const stored=localStorage.getItem("jokes-story");if(stored)story=JSON.parse(stored) as Story}catch{}story={...story,friends:story.friends.map(f=>f.id===1?{...f,name:"Thalaivar",image:f.image?.startsWith("data:")?f.image:thalaivar.image,specialImage:f.specialImage?.startsWith("data:")?f.specialImage:thalaivar.chapterImage,memoryImage:f.memoryImage?.startsWith("data:")?f.memoryImage:thalaivar.memoryImage,chapterImage:f.chapterImage?.startsWith("data:")?f.chapterImage:thalaivar.chapterImage,cornerImage:f.cornerImage?.startsWith("data:")?f.cornerImage:thalaivar.cornerImage}:f)};localStorage.setItem("jokes-story",JSON.stringify(story));setGroup(story.group);setFriend(story.friends.find(item=>item.id===id)??fallback.friends.find(item=>item.id===id)??fallback.friends[0]);setReady(true)},[]);
 ...
 return <main className="folder-page">
  ...
  <section className="chapter-intro"><p><span/> FRIENDSHIP CHAPTER {String(friend.id).padStart(2,"0")}</p>{friend.cornerImage&&<figure className="corner-visual"><img src={friend.cornerImage} alt={`A little corner with ${friend.name}`}/><figcaption>{friend.name} • Little corner</figcaption></figure>}<h1>A little corner<br/>made for <em>{friend.name}.</em></h1><div><small>{friend.nickname}</small><span>{active?"Read it fully, then place it back inside.":"Select either paper to open it completely ↓"}</span></div></section>
  <section className={`memory-stage ${active?`is-open ${active}-active`:""}`} aria-label={`Interactive memory folder for ${friend.name}`}>
   ...
   <article className="paper photo-paper" role="button" tabIndex={0} aria-pressed={active==="photo"} onClick={()=>setActive("photo")} onKeyDown={openWithKeyboard("photo")}>
    <button className="paper-return" onClick={event=>{event.stopPropagation();setActive(null)}}><span>↓</span> Place back in folder</button>
    <div className="tape"/><div className="special-photo" style={{backgroundColor:friend.color}}>{(friend.specialImage||friend.chapterImage)?<img src={friend.specialImage||friend.chapterImage} alt={`A special memory with ${friend.name}`}/>:friend.image?<img src={friend.image} alt={friend.name}/>:<span>{marks}</span>}</div><div className="photo-caption"><p>A frame worth keeping forever.</p><small>{friend.specialImage?"OUR SPECIAL MEMORY":"Add a special image in Edit story"}</small></div><span className="paper-action">Click to view the full photograph ↗</span>
   </article>
